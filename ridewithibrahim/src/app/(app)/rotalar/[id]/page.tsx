...
export default async function RouteDetailPage({
  ...
  const { data: rawComments } = await supabase
    ...
    .limit(100);
  const cRows = rawComments ?? [];

  let commentAuthors = new Map<string, string>();
  if (cRows.length) {
    const uids = [...new Set(cRows.map((c) => c.user_id as string))];
    const { data: cp } = await supabase.from("profiles").select("id, username").in("id", uids);
    commentAuthors = new Map((cp ?? []).map((p) => [p.id as string, p.username as string]));
  }
  const comments = cRows.map((c) => ({
    id: c.id as string,
    content: c.content as string,
    createdAt: c.created_at as string,
    author: commentAuthors.get(c.user_id as string) ?? "kullanıcı",
  }));
